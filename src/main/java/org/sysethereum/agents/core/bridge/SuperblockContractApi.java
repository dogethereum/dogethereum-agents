...
public class SuperblockContractApi {
    ...
    public void challengeCancelTransfer(BlockSPVProof blockSPVProof, SuperblockSPVProof superblockSPVProof){
        ...
        CompletableFuture<TransactionReceipt> futureReceipt = superblocksForChallenges.challengeCancelTransfer(new DynamicBytes(BaseEncoding.base16().decode(blockSPVProof.transaction.toUpperCase())), new Uint256(blockSPVProof.index),new DynamicArray<Uint256>(txSiblings),
                new DynamicBytes(BaseEncoding.base16().decode(blockSPVProof.header.toUpperCase())), new Uint256(superblockSPVProof.index), new DynamicArray<Uint256>(blockSiblings), new Bytes32(BaseEncoding.base16().decode(superblockSPVProof.superBlock))).sendAsync();
